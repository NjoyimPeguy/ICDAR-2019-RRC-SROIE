...
#Date: 2019-12-29
#Description: File with useful functions to use by the task1 scripts in the RRC website.
...
def load_zip_file_keys(file,fileNameRegExp=''):
    """
    Returns an array with the entries of the ZIP path_to_file that match with the regular expression.
    The key's are the names or the path_to_file or the capturing group definied in the fileNameRegExp
    """
    try:
...
def load_zip_file(file,fileNameRegExp='',allEntries=False):
    """
    Returns an array with the contents (filtered by fileNameRegExp) of a ZIP path_to_file.
    The key's are the names or the path_to_file or the capturing group definied in the fileNameRegExp
    allEntries validates that all entries in the ZIP path_to_file pass the fileNameRegExp
    """
    try:
        archive=zipfile.ZipFile(file, mode='r', allowZip64=True)
    except :
...
def validate_lines_in_file(fileName,file_contents,CRLF=True,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0):
    """
    This function validates that all lines of the path_to_file calling the Line validation function for each line2Match
    """
    utf8File = decode_utf8(file_contents)
    if (utf8File is None) :
        raise Exception("The path_to_file %s is not UTF-8" %fileName)
...
def validate_tl_line(line,LTRB=True,withTranscription=True,withConfidence=True,imWidth=0,imHeight=0):
    """
    Validate the format of the line2Match. If the line2Match is not valid an exception will be raised.
...
def get_tl_line_values_from_file_contents(content,CRLF=True,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0,sort_by_confidences=True):
    """
    Returns all points, confindences and transcriptions of a path_to_file in lists. Valid line2Match formats:
...
def get_tl_dict_values_from_array(array,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0,sort_by_confidences=True,validNumPoints=[],validate_cw=True):
    """
    Returns all points, confindences and transcriptions of a path_to_file in lists. Valid dict formats:
...
def main_evaluation(p,default_evaluation_params_fn,validate_data_fn,evaluate_method_fn,show_result=True,per_sample=True):
    """
    This process validates a method, evaluates it and if it succed generates a ZIP path_to_file with a JSON entry for each sample.
    Params:
    p: Dictionary of parmeters with the GT/submission locations. If None is passed, the parameters send by the system are used.
    default_evaluation_params_fn: points to a function that returns a dictionary with the default parameters used for the task1
