...
def evaluation_imports():
    """
    evaluation_imports: Dictionary ( key = module name , value = alias  )  with python modules used in the task1.
    """    
    return {
            'math':'math',
            'numpy':'np'
...
def default_evaluation_params():
    """
    default_evaluation_params: Default parameters to use for the validation and task1.
    """
    return {
...
def validate_data(gtFilePath, submFilePath,evaluationParams):
    ...
    if len(subm) != len(gt) :
        raise Exception("The Det path_to_file is not valid (invalid number of files in ZIP. Expected:" + str(len(gt)) + " Found:" + str(len(subm)) + ")")
